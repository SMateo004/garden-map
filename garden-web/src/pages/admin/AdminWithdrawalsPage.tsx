  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Rechazar retiro</h3>
        <p className="text-sm text-gray-500 mb-4">El cuidador recibirá este motivo en su notificación.</p>
        <textarea
          value={reason}
          onChange={e => setReason(e.target.value)}
          placeholder="Motivo del rechazo…"
          rows={3}
          className="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white px-4 py-3 text-sm focus:ring-2 focus:ring-red-500 outline-none resize-none"
        />
        <div className="flex gap-3 mt-4">
          <button
            onClick={onCancel}
            className="flex-1 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-bold text-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition-all"
          >
            Cancelar
          </button>
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white font-bold text-sm transition-all"
          >
...
export function AdminWithdrawalsPage() {
  ...
  const completeMutation = useMutation({
    ...
    onError: (e: any) => toast.error(e?.response?.data?.error?.message || e.message || 'Error al completar'),
  });
    ...
    <>
      {rejectingId && (
        <RejectModal
          ...
          onConfirm={(reason) => rejectMutation.mutate({ id: rejectingId, reason })}
                ...
                <div className="space-y-2">
                  ...
                  {w.user.caregiverProfile && (
                    <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-100 dark:border-gray-700 text-xs grid grid-cols-2 gap-x-4 gap-y-1">
                      ...
                      <span className="text-gray-400">Saldo actual:</span> <span className="font-bold text-green-600">Bs {w.user.caregiverProfile.balance}</span>
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-2 self-center shrink-0 min-w-[200px]">
                  {w.status === 'PENDING' && (
                    <button
                      ...
                      className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/20 transition-all hover:scale-105 active:scale-95"
                    >
                      Procesar Retiro
                    </button>
                  )}
                  {(w.status === 'PENDING' || w.status === 'PROCESSING') && (
                    <button
                      ...
                      className="w-full py-3 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white rounded-xl font-bold text-sm shadow-lg shadow-green-500/20 transition-all hover:scale-105 active:scale-95"
                    >
                      Marcar Completado
                    </button>
                  )}
