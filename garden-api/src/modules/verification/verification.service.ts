...
export async function generateLink(userId: string): Promise<{ url: string; token: string; expiresIn: string }> {
  ...
  const appShell = baseUrl.includes('localhost') ? '' : '/app.html';
  const url = `${baseUrl}${appShell}#/mobile-verify?token=${encodeURIComponent(token)}`;
      ...
      await prisma.$transaction([
        prisma.identityVerificationSession.update({
          ...
          data: {
            ...
            deviceFingerprint: fingerprint?.substring(0, 128),
