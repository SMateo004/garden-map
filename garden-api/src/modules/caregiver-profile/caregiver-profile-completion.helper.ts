...
export async function checkAndAutoSubmitProfile(userId: string) {
    ...
    if (isTotalComplete && profile.status === 'DRAFT') {
        logger.info('Perfil completo, activando auto-aprobación...', { userId });
        try {
            const { submitProfile } = await import('./caregiver-profile.service.js');
            await submitProfile(userId);
        } catch (err) {
            logger.error('Error en auto-submit automático', { userId, err });
        }
    }
}
